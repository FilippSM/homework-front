...
export const HW3 = () => {
  ...
  const handleSave = () => {
    setTexts([...texts, currentText]) //не используется push так как он изменяет исходный массив React не сможет определить, что состояние изменилось, так как ссылка на массив texts не изменилась Использование оператора расширения (...) позволяет создать новый массив, который включает все элементы старого массива, а также новый элемент
    setCurrentText("")
